...
def create_use_sensitivity_figure():
    ...
    def scale_line_thickness(capacity_gw, data_min=0.00000001, data_max=1000, viz_min=5, viz_max=60.0):
...
def create_stock_sensitivity_figure():
    ...
    ax.legend(handles=legend_elements, loc='lower left', fontsize=12, title="Baseline Capacity Legend",title_fontsize=14)
...
def build_interconnection_summary_table(df_stock, df_use):
    ...
    def label_disappearance(value):
        if pd.isna(value):
            return value
        return ">1" if value > 50000 else f"{value:.2f}"
